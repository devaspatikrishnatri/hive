...
public class TestReplicationScenarios {
 ...
 static class Tuple {
  ...
  @Test
  public void testBootstrapWithDataInDumpDir() throws IOException, TException {
    ...
    Path dbPath = new Path(db.getManagedLocationUri());
    Path tableRelativeSrcPath = new Path(dbPath, "unptned");
    Path srcFileLocation = new Path(tableRelativeSrcPath, unptnedFileName1);
    ...
    srcFileLocation = new Path(tableRelativeSrcPath, unptnedFileName2);
    verifyChecksum(srcFileLocation, tgtFileLocation, false);

    tableRelativeSrcPath = new Path(dbPath, "ptned");
    Path partitionRelativeSrcPath = new Path(tableRelativeSrcPath,  "b=1");
    srcFileLocation = new Path(partitionRelativeSrcPath, ptnedFileName1);
    ...
    verifyChecksum(srcFileLocation, tgtFileLocation, true);
    partitionRelativeSrcPath = new Path(tableRelativeSrcPath,  "b=2");
    srcFileLocation = new Path(partitionRelativeSrcPath, ptnedFileName2);
    loadAndVerify(replDbName, dbName, dump.lastReplId);
    verifyChecksum(srcFileLocation, tgtFileLocation, false);
  ...
  public void testDumpWithPartitionDirMissing() throws IOException {
    ...
    FileSystem fs = path.getFileSystem(hconf);
    LOG.info("DELETING:partition=" + path);
    fs.delete(path);
