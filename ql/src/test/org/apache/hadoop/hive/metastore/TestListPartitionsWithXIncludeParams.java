...
public class TestListPartitionsWithXIncludeParams
  ...
  public void testGetPartitionsByNames() throws Exception {
    Table t = createTablePartColsParts(getClient()).table;
    List<String> part_names = Arrays.asList("yyyy=1999/mm=01/dd=02",
        "yyyy=2009/mm=02/dd=10", "yyyy=1999/mm=03/dd=02");
    List<Partition> partitions =
        getClient().getPartitionsByNames(t.getCatName(), t.getDbName(), t.getTableName(), part_names);
    ...
    partitions = getClient().getPartitionsByNames(t.getCatName(), t.getDbName(), t.getTableName(),
        part_names);
  ...
  public void testGetPartitionsRequest() throws Exception {
    ReturnTable returnTable = createTablePartColsParts(getClient());
    ...
    List<Partition> partitions = MetaStoreUtils.getPartitionsByProjectSpec(getClient(), request);
    assertPartitionsHaveCorrectParams(partitions);
    List<List<String>> values = partitions.stream().map(partition -> partition.getValues()).collect(Collectors.toList());
    ...
    partitions = MetaStoreUtils.getPartitionsByProjectSpec(getClient(), request);
    assertPartitionsHaveCorrectParams(partitions);
