...
public class TestDbTxnManager2 {
  ...
  public void testMergeMultipleBranchesOptimistic() throws Exception {
    ...
    try {
      ...
      Assert.assertTrue(ex.getErrorMessage().matches(".*Aborting .* due to a write conflict on default/target.*"));
      Assert.assertEquals(ErrorMsg.TXN_ABORTED.getErrorCode(), ex.getErrorCode());
    }
    swapTxnManager(txnMgr);
  ...
  public void testConcurrentConflictMaxRetryCount() throws Exception {
    ...
    try {
      ...
      Assert.assertNotNull(cpe.getErrorMessage());
      exceptionThrown = true;
      Assert.assertTrue(
          cpe.getErrorMessage().contains("Operation could not be executed, snapshot was outdated when locks were acquired."));
    }
  ...
  public void testFairness2() throws Exception {
    ...
    ((DbLockManager)txnMgr2.getLockManager()).checkLock(locks.get(4).getLockid());
    locks = getLocks();
    Assert.assertEquals("Unexpected lock count", 3, locks.size());
  ...
  public void testRemoveDuplicateCompletedTxnComponents() throws Exception {
    ...
    Assert.assertEquals(TxnDbUtil.queryToString(conf, "select * from \"COMPLETED_TXN_COMPONENTS\""),
      5, TxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\""));
    ...
    Assert.assertEquals(1, TxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" " +
    ...
    Assert.assertEquals(2, TxnDbUtil.countQueryAgent(conf, "select count(*) from \"COMPLETED_TXN_COMPONENTS\" " +
