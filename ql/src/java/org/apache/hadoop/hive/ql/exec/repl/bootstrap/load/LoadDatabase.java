...
public class LoadDatabase {
  ...
  private Task<? extends Serializable> createDbTask(Database dbObj) throws MetaException {
    // note that we do not set location - for repl load, we want that auto-created.
    CreateDatabaseDesc createDbDesc = new CreateDatabaseDesc(dbObj.getName(), dbObj.getDescription(),
            getDbLocation(dbObj), false, updateDbProps(dbObj, context.dumpDirectory), getDbManagedLocation(dbObj));
