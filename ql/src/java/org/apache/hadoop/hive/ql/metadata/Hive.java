...
public class Hive {
  ...
  public static void listFilesInsideAcidDirectory(Path acidDir, FileSystem srcFs, List<FileStatus> newFiles, PathFilter filter)
          throws IOException {
...
private void constructOneLBLocationMap(FileStatus fSta,
    ...
    try {
      ...
      if (!isMmTable && !isDirectInsert) {
        ...
        Path[] leafStatus = Utilities.getDirectInsertDirectoryCandidates(
          fs, loadPath, numDP, null, writeId, -1, conf, isInsertOverwrite, operation);
  ...
  public Map<Map<String, String>, Partition> loadDynamicPartitions(
    ...
    tblDesc.setProperties(properties);

    LoadTableDesc tbd = new LoadTableDesc(loadPath, tblDesc, partSpec, loadFileType, 0L);
    tbd.setInsertOverwrite(isInsertOverwrite);
    tbd.setIsDirectInsert(isDirectInsert);
