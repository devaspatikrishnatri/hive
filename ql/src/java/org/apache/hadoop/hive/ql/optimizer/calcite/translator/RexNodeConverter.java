        ...
        if (RexUtil.isNull(child)) {
          if (i % 2 == 0 && i != childRexNodeLst.size() - 1) {
            if (SqlTypeName.NULL.equals(child.getType().getSqlTypeName())) {
              child = cluster.getRexBuilder().makeNullLiteral(
                      cluster.getRexBuilder().getTypeFactory().createSqlType(SqlTypeName.BOOLEAN));
            }
          } else {
            ...
            if (!RexUtil.isNull(childRexNodeLst.get(1))) {
              child = cluster.getRexBuilder().makeCast(childRexNodeLst.get(1).getType(), child);
            }
          }
        }
