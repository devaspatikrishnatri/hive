...
public class CalcitePlanner extends SemanticAnalyzer {
  ...
  public static RelOptPlanner createPlanner(HiveConf conf) {
    return createPlanner(conf, new HashSet<>(), new HashSet<>(), false);
  }

  private static RelOptPlanner createPlanner(
      HiveConf conf, Set<RelNode> corrScalarRexSQWithAgg, Set<RelNode> scalarAggNoGbyNoWin,  boolean isExplainPlan) {
  ...
  private class CalcitePlannerAction implements Frameworks.PlannerAction<RelNode> {
    ...
    public RelNode apply(RelOptCluster cluster, RelOptSchema relOptSchema, SchemaPlus rootSchema) {
       ...
       */
      RelOptPlanner planner = createPlanner(conf, corrScalarRexSQWithAgg, scalarAggNoGbyNoWin, ctx.isExplainPlan());
      ...
      if (profilesCBO.contains(ExtendedCBOProfile.WINDOWING_POSTPROCESSING)) {
        ...
        calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, false, mdProvider.getMetadataProvider(), null,
                HepMatchOrder.DEPTH_FIRST, HiveWindowingLastValueRewrite.INSTANCE);
      ...
      perfLogger.PerfLogBegin(this.getClass().getName(), PerfLogger.OPTIMIZER);
      calciteOptimizedPlan = hepPlan(calciteOptimizedPlan, false, mdProvider.getMetadataProvider(), null,
          HepMatchOrder.BOTTOM_UP, HiveDruidPullInvertFromBetweenRule.INSTANCE,
      ...
      if (leftSemiJoin) {
        ...
        topRel = HiveSemiJoin.getSemiJoin(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION),
            ...
            HiveCalciteUtil.fixNullability(cluster.getRexBuilder(),
                calciteJoinCond, RelOptUtil.getFieldTypeList(combinedRowType)), ImmutableIntList.copyOf(leftKeys),
    ...
    private RelNode genFilterRelNode(ASTNode filterExpr, RelNode srcRel,
      ...
      RexNode filterCondn = genRexNode(filterExpr, relToHiveRR.get(srcRel),
          outerRR, null, useCaching, cluster.getRexBuilder());

      return genFilterRelNode(filterCondn, srcRel, outerNameToPosMap, outerRR);
    }

    private RelNode genFilterRelNode(RexNode filterCondn, RelNode srcRel,
        ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR) throws SemanticException {
      if (RexUtil.isLiteral(filterCondn, false)
          && filterCondn.getType().getSqlTypeName() != SqlTypeName.BOOLEAN) {
      ...
      ImmutableMap<String, Integer> hiveColNameCalcitePosMap = this.relToHiveColNameCalcitePosMap
          .get(srcRel);
      filterCondn = new CorrelationConverter(
          new InputContext(srcRel.getRowType(), hiveColNameCalcitePosMap, relToHiveRR.get(srcRel)),
          outerNameToPosMap, outerRR, subqueryId).apply(filterCondn);
      RexNode factoredFilterExpr = RexUtil
          .pullFactors(cluster.getRexBuilder(), filterCondn);
      RelNode filterRel = new HiveFilter(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION),
          srcRel, HiveCalciteUtil.fixNullability(cluster.getRexBuilder(),
          factoredFilterExpr, RelOptUtil.getFieldTypeList(srcRel.getRowType())));
    ...
    private class CorrelationConverter extends RexShuttle {
      ...
      private CorrelationConverter(InputContext inputContext, ImmutableMap<String, Integer> outerColumnNameToPosition,
          RowResolver outerRowResolver, int correlatedId) {
        this.inputContext = inputContext;
        this.outerPositionToColumnName =
            outerColumnNameToPosition == null ? null : ImmutableBiMap.copyOf(outerColumnNameToPosition).inverse();
      ...
      public RexNode visitInputRef(RexInputRef col) {
        ...
        return cluster.getRexBuilder().makeInputRef(context.inputRowType.getFieldList().get(pos).getType(), pos);
      }
    }
