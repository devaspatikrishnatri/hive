...
public class CopyUtils {
  ...
  String checkSumFor(Path srcFile, FileSystem fs) throws IOException {
    return retryableFxn(new Callable<String>() {
      @Override
      public String call() throws Exception {
        return ReplChangeManager.checksumFor(srcFile, fs);
      }
    });
  }
  ...
  boolean exists(FileSystem fs, Path path) throws IOException {
    return retryableFxn(new Callable<Boolean>() {
      @Override
  ...
  ContentSummary getContentSummary(FileSystem fs, Path f) throws IOException {
    return retryableFxn(new Callable<ContentSummary>() {
      @Override
