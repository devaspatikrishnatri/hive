...
abstract class TxnHandler implements TxnStore, TxnStore.MutexAPI {
  ...
  private OpenTxnList getOpenTxnsList(boolean infoFields) throws MetaException {
    try {
      ...
      try {
        ...
        String txnsQuery = String.format(infoFields ? OpenTxn.OPEN_TXNS_INFO_QUERY : OpenTxn.OPEN_TXNS_QUERY, hwm);
  ...
  private List<Long> openTxns(Connection dbConn, Statement stmt, OpenTxnRequest rqst)
    ...
    try {
      ...
      for (long i = first; i < first + numTxns; i++) {
        txnIds.add(i);
        rows.add(i + "," + TxnStatus.OPEN  + "," + TxnDbUtil.getEpochFn(dbProduct)  + ","
      ...
      if (txnIds.size() == 1) {
        s = "INSERT INTO \"MIN_HISTORY_LEVEL\" (\"MHL_TXNID\", \"MHL_MIN_OPEN_TXNID\") " +
                "SELECT ?, MIN(\"TXN_ID\") FROM \"TXNS\" WHERE \"TXN_STATE\" = " + TxnStatus.OPEN;
        LOG.debug("Going to execute query <" + s + ">");
        ...
        s = "SELECT MIN(\"TXN_ID\") FROM \"TXNS\" WHERE \"TXN_STATE\" = " + TxnStatus.OPEN;
        LOG.debug("Going to execute query <" + s + ">");
        ...
        if (txnid > 0) {
          ...
          for (LockComponent lc : rqst.getComponent()) {
            ...
            rows.add(txnid + ", ?, " +
                    (tblName == null ? "null" : "?") + ", " +
                    (partName == null ? "null" : "?")+ "," +
                    OperationType.fromDataOperationType(lc.getOperationType()).toString()+ "," +
  ...
  public ShowCompactResponse showCompact(ShowCompactRequest rqst) throws MetaException {
    ...
    try {
      try {
        ...
        //what I want is order by cc_end desc, cc_start asc
        // (but derby has a bug https://issues.apache.org/jira/browse/DERBY-6013)
    ...
    try {
      try {
        ...
        for (String partName : rqst.getPartitionnames()) {
          rows.add(rqst.getTxnid() + ",?,?,?," + ot + "," + writeId);
          List<String> params = new ArrayList<>();
  ...
  private int abortTxns(Connection dbConn, List<Long> txnids, boolean checkHeartbeat, boolean isStrict)
    ...
    try {
      ...
      prefix.append("UPDATE \"TXNS\" SET \"TXN_STATE\" = " + TxnStatus.ABORTED +
        " WHERE \"TXN_STATE\" = " + TxnStatus.OPEN + " AND ");
      if(checkHeartbeat) {
  ...
  private void timeOutLocks(Connection dbConn) {
    ...
    try {
      ...
      String s = "SELECT DISTINCT \"HL_LOCK_EXT_ID\" FROM \"HIVE_LOCKS\" WHERE \"HL_LAST_HEARTBEAT\" < " +
          TxnDbUtil.getEpochFn(dbProduct) + "-" + timeout + " AND \"HL_TXNID\" = 0";//when txnid is <> 0, the lock is
